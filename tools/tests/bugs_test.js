...
console.log("\nSC — comprobación puntual de actividad solar (Vuelo único)");
{
  ...
  ok("SC15 la fecha/hora de la fila no se aplasta (baja entera si no cabe)",
     rowSrc.indexOf('className: "gle-dep gle-dep-inline"') !== -1 &&
     html.indexOf('.gle-field-date { flex: 0 0 122px; }') !== -1 &&
  ...
  ok("SC15 la fila se mantiene legible y sin aplastar los campos",
     /\.gle-field-date \{ flex: 0 0 122px; \}/.test(html) &&
     /\.gle-field-time \{ flex: 0 0 92px; \}/.test(html) &&
     /\.gle-act \{[^}]*padding:0 9px/.test(html));
  ok("SC15 fecha, hora y Cálculo van juntos y bajan enteros si no caben",
     /flex: "0 0 auto"/.test(rowSrc) && /flexWrap: "nowrap"/.test(rowSrc) &&
     rowSrc.indexOf('className: "gle-dep gle-dep-inline"') !== -1);
